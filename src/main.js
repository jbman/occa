...
async function main() {
    const {keyPair, publicJwk} = await KeyPairGenerator.generateRS256KeyPairForDPoP();
    ...
    Refresh.init(
        {
            ...
            onResponse: (response) => {
                ...
                Render.notifySuccess("Refresh complete, tokens updated.", ELEMENTS.refreshNotification);
            },
...
// Check if code is provided on callback from auth server at "/code" or 
    ...
    if (code) {
        ...
        if (config.dpopEnabled) {
            const dpop = await DPoPJwtGenerator.generateDpopJwt(httpMethod, tokenEndpoint, keyPair, publicJwk, null, null);
