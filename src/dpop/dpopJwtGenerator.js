 ...
 */
async function generateDpopJwt(
  bindToHttpMethod,
  bindToTargetUri,
  keyPair,
  publicJwk,
  ...
  const body = await _generateBody(
    bindToHttpMethod,
    bindToTargetUri,
    access_token,
    nonce,
  ...
  const tokenWithoutSig =
    base64UrlEncode(textEncoder.encode(header)) +
    "." +
    base64UrlEncode(textEncoder.encode(body));
